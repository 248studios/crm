...
from crm.aoscrm.doctype.crm_service_level_agreement.utils import get_sla
from crm.aoscrm.doctype.crm_status_change_log.crm_status_change_log import add_status_change_log
from crm.aoscrm.doctype.aoscrm_settings.aoscrm_settings import get_exchange_rate
...
class CRMDeal(Document):
	...
	def update_expected_deal_value(self):
		...
		"""
		if (
			frappe.db.get_single_value("AOSCRM Settings", "auto_update_expected_deal_value")
	...
	def validate_forecasting_fields(self):
		...
		if frappe.db.get_single_value("AOSCRM Settings", "enable_forecasting"):
	...
	def update_exchange_rate(self):
		if self.has_value_changed("currency") or not self.exchange_rate:
			system_currency = frappe.db.get_single_value("AOSCRM Settings", "currency") or "USD"
			exchange_rate = 1
