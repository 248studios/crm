...
async function setupForm({ doc, call, $dialog, updateField, toast }) {
		...
		actions.push({
			...
			onClick: async () => {
				let quotation_url = await call(
					"crm.aoscrm.doctype.erpnext_crm_settings.erpnext_crm_settings.get_quotation_url", 
					{
						crm_deal: doc.name,
						organization: doc.organization
					}
	...
	if (is_erpnext_integration_enabled) {
		let customer_url = await call("crm.aoscrm.doctype.erpnext_crm_settings.erpnext_crm_settings.get_customer_link", {
			crm_deal: doc.name
		});
		if (customer_url) {
			actions.push({
