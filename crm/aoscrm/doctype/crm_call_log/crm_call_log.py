...
def get_call_log(name):
	...
	if call.get("note"):
		note = frappe.get_cached_doc("AOSCRM Note", call.get("note")).as_dict()
		notes.append(note)
	...
	if call.get("links"):
		for link in call.get("links"):
			if link.get("link_doctype") == "CRM Task":
				...
				tasks.append(task)
			elif link.get("link_doctype") == "AOSCRM Note":
				note = frappe.get_cached_doc("AOSCRM Note", link.get("link_name")).as_dict()
				notes.append(note)
